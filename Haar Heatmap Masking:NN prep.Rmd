...
Differences3X3[2,] <- Differences3X3[1,]
Differences3X3[4,] <- Differences3X3[5,]
Differences3X3[3,] <- (colSums(Differences3X3[2:4,])/3)

Differences<- abs(Differences3X3)
heatmap.2(Differences,
  main = "3x3 Kirsch Averaged ",
  xlab = "Time",
  ylab = "Frequency",
  key=T, 
  keysize=1.5, 
...
```{r}
k<-3
Differences5X5 <- Differences
a<- matrix(c(0),5,5)

horiz<- t(as.matrix(c(9,9,9,9,9,9,5,5,5,9,-7,-3,0,-3,-7,-7,-3,-3,-3,-7,-7,-7,-7,-7,-7)))
    ...
    for (r in 1:5){
        a[r,] <-c(Differences[r,(i-2):(i+2)]) 
            Z<-c(a[k-2,],a[k-1,],a[k,],a[k+1,],a[k+2,])
                Differences5X5[3,i] <- horiz %*% Z
  }
}

Differences<- abs(Differences5X5)
heatmap.2(Differences,
  main = "5x5 Kirsch ",
  xlab = "Time",
  ylab = "Frequency",
